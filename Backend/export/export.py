...
from export.depends import serialize_model
...
def export_json_data(
        water_body_ids: Optional[str] = Query(None, description="Список ID водоемов через запятую"),
        db: Session = Depends(get_db)
):
    query = db.query(models.WaterBody).options(
        joinedload(models.WaterBody.organisms).joinedload(models.Organism.organism_type),
